...
import validation_utils  # validate_utils 모듈 임포트
...
def extract_features(df, COLUMNS_CHART_DATA):
    try:
        ...
        # 날짜 형식을 pd.Timestamp로 변환
        df['date'] = pd.to_datetime(df['date'])

        # 상한가 조건 추가
        limit_up_date = pd.Timestamp('2015-06-01')
        df['is_limit_up'] = ((df['close'] / df['open'] - 1) * 100 >= 30) & (df['date'] >= limit_up_date)
        df['is_limit_up'] |= ((df['close'] / df['open'] - 1) * 100 >= 15) & (df['date'] < limit_up_date)
        
        # 상한가 종목 제외
        df = df[~df['is_limit_up']]
    ...
    try:
        ...
        for _, row in deep_learning_df.iterrows():
            insert_query = f"""
                ...
                VALUES ('{row['date']}', '{row['method']}', '{row['stock_name']}', {row['confidence']}, {row['estimated_profit_rate']})
            """
...
def setup_environment():
    ...
    settings = {
        ...
        'model_name': 'breakthrough'  # 추가: 모델 이름
    }
...
def predict_pattern_with_score(model, df, stock_name, use_data_dates=False, settings=None):
    ...
    print(f"Required columns: {COLUMNS_TRAINING_DATA}")
    missing_columns = [col for col in COLUMNS_TRAINING_DATA if col not in df.columns]
    if missing_columns:
    ...
    try:
        ...
        print(f"Inf values in X:\n{np.isinf(X).sum()}")
        X = X.replace([np.inf, -np.inf], np.nan).dropna()
        print(f"Shape of X after cleaning: {X.shape}")
        
        if X.empty:
            print("X is empty after cleaning. Skipping prediction.")
            return pd.DataFrame(columns=['date', 'stock_name', 'Score']), 0
