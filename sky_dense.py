...
def label_data(df, signal_dates):
    try:
        ...
        # 마지막 시그널 날짜만 사용
        if valid_signal_dates:
            last_signal_date = valid_signal_dates[-1]
            print(f'Using last signal date: {last_signal_date}')
            
            # 1. last_signal_date를 기준으로 라벨 2 설정
            df.loc[df['date'] == last_signal_date, 'Label'] = 2
            
            # 2. last_signal_date 이후 7일 동안 종가가 last_signal_date 종가의 5% 이하인 경우 라벨 2로 설정
            signal_rows = df[df['date'] == last_signal_date]
            if not signal_rows.empty:
                signal_close = signal_rows['close'].values[0]
                threshold = signal_close * 1.05  # 5% 이하
                date_range = (df['date'] > last_signal_date) & (df['date'] <= last_signal_date + timedelta(days=7))
                df.loc[date_range & (df['close'] <= threshold), 'Label'] = 2
            
            # 3. 최소 라벨 2 개수가 5개가 되도록 부족분을 last_signal_date 이전으로 분배
            label_2_count = len(df[df['Label'] == 2])
            if label_2_count < 5:
                additional_needed = 5 - label_2_count
                previous_dates = df[df['date'] < last_signal_date].sort_values(by='date', ascending=False)
            ...
            # 4. last_signal_date 이전 5봉을 라벨 1로 설정
            previous_dates = df[df['date'] < last_signal_date].sort_values(by='date', ascending=False)
            if not previous_dates.empty:
