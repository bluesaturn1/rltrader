...
def validate_model(settings, model):
    ...
    if validation_results:
        ...
        print("다음을 확인해 보세요:")
        print("1. 검증 기간이 적절한가? (cf.VALIDATION_START_DATE_AUTO, cf.VALIDATION_END_DATE_AUTO)")
...
def setup_validation_environment(settings):
    ...
    # 검증 날짜 범위 설정
    validation_start = datetime.strptime(cf.VALIDATION_START_DATE_AUTO, '%Y%m%d').date()
    validation_end = datetime.strptime(cf.VALIDATION_END_DATE_AUTO, '%Y%m%d').date()
