    ...
    for index, row in tqdm(validation_results.iterrows(), total=len(validation_results), desc="성능 평가", position=0):
        ...
        if not df.empty:
            ...
            if perf_result is not None:
                ...
                print(f"No valid return found for {stock_name} from {performance_start_date} to {performance_end_date}")
            ...
            print(f"No data loaded for {stock_name} from {performance_start_date} to {performance_end_date}")
    ...
    try:
        ...
        if check_result.empty:
            ...
            CREATE TABLE `{table}` (
                ...
                PRIMARY KEY (`id`),
                UNIQUE KEY `unique_record` (`stock_name`,`date`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
            """
            db_manager.execute_query(create_table_query)
            print(f"Table {table} created successfully")
            table_columns = ['id', 'stock_name', 'date', 'prediction', 'start_date', 'end_date', 'max_return', 'max_return_day', 'max_loss', 'max_loss_day', 'estimated_profit_rate']
        else:
            # 테이블 구조 확인
        ...
        existing_query = f"""
            SELECT DISTINCT stock_name, {date_column} FROM {table}
        """
        ...
        if not existing_data.empty:
            ...
            for idx, row in df.iterrows():
                ...
                if pd.notnull(row[date_column_in_df]):
                    ...
                    pair = (row['stock_name'], date_value)
...
def process_validation_results(validation_results, settings, model):
    ...
    # 데이터프레임 생성 시 컬럼 지정
    results_df = pd.DataFrame(validation_results, columns=['stock_name', 'date', 'confidence', 'action', 'max_profit_rate', 'max_loss_rate', 'max_profit_date', 'max_loss_date', 'estimated_profit_rate'])
...
def calculate_top3_performance(results_df, craw_db, analysis_period=15):
    """
    각 날짜별로 예측 신뢰도가 가장 높은 상위 3개 종목의 성과를 분석합니다.
    
    Args:
        results_df: 예측 결과가 담긴 데이터프레임
        craw_db: 주가 데이터 데이터베이스 연결
        analysis_period: 성과 측정 기간 (일)
        
    Returns:
        각 날짜별 상위 3개 종목의 평균 성과를 포함한 데이터프레임
    """
    try:
        if results_df.empty:
            print("성과를 계산할 결과가 없습니다.")
            return None
            
        # 날짜 형식 통일
        results_df['pattern_date'] = pd.to_datetime(results_df['pattern_date'])
        
        # 날짜별로 그룹화
        date_groups = results_df.groupby(results_df['pattern_date'].dt.date)
        
        performance_results = []
        
        for date, group in date_groups:
            # 예측 신뢰도(prediction) 기준 상위 3개 종목 선택
            top3_stocks = group.nlargest(3, 'prediction')
            ...
            # 종목별 성과 지표 계산
            for idx, stock in top3_stocks.iterrows():
                stock_name = stock['stock_name']
                pattern_date = stock['pattern_date']
                
                # 매수일(pattern_date 다음날)과 성과 측정 종료일 계산
                buy_date = pattern_date + pd.Timedelta(days=1)
                end_date = buy_date + pd.Timedelta(days=analysis_period)
                
                # 해당 기간 데이터 조회
                stock_data = load_daily_craw_data(craw_db, stock_name, buy_date, end_date)
                ...
                if not stock_data.empty and len(stock_data) >= 2:
                    ...
                    if buy_price > 0:
                        # 일별 수익률 계산
                        stock_data['return'] = (stock_data['close'] - buy_price) / buy_price * 100
                        
                        # 최대 상승률과 최대 하락률
                        max_return = stock_data['return'].max()
                        max_loss = stock_data['return'].min()
                        
                        # 위험 조정 수익률
                        risk_adjusted = max_return - abs(max_loss)
            ...
            # 평균 계산 (0으로 나누기 방지)
            n_stocks = len(top3_stocks)
            if n_stocks > 0:
                avg_max_return = total_max_return / n_stocks
                avg_max_loss = total_max_loss / n_stocks
                avg_risk_adjusted = total_risk_adjusted / n_stocks
                ...
                performance_results.append({
                    ...
                    'top_stock': top3_stocks.iloc[0]['stock_name'] if len(top3_stocks) > 0 else None,
                    'top_confidence': top3_stocks.iloc[0]['prediction'] if len(top3_stocks) > 0 else 0
                })
                
        if performance_results:
            return pd.DataFrame(performance_results)
        else:
            print("계산된 성과 결과가 없습니다.")
            return None
            
    except Exception as e:
        print(f"Top 3 성과 계산 중 오류 발생: {e}")
        import traceback
        traceback.print_exc()
        return None
