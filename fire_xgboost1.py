    ...
    try:
        ...
        if model is not None:
            ...
            if hasattr(model, 'predict_proba'):
                ...
                # 임계값 최적화 - 재현율 우선
                print("\n재현율 최적화 임계값 찾기:")
                threshold_recall = optimize_threshold(model, X_test, y_test_binary, metric='recall')
                
                # 정밀도가 너무 낮아지지 않도록 제한 (예: 0.3 이상)
                recall_results = []
                for t in np.arange(0.1, threshold_recall+0.05, 0.05):
                    y_pred_t = (y_pred_proba >= t).astype(int)
                    prec = precision_score(y_test_binary, y_pred_t)
                    rec = recall_score(y_test_binary, y_pred_t)
                    if prec >= 0.3:  # 정밀도 최소 기준
                        recall_results.append((t, prec, rec))
                
                if recall_results:
                    # 정밀도 제한 내에서 재현율이 가장 높은 임계값 선택
                    best_threshold = max(recall_results, key=lambda x: x[2])[0]
                    print(f"\n정밀도 0.3 이상 유지하면서 최적 임계값: {best_threshold}")
                else:
                    # F1 점수로 대체
                    best_threshold = optimize_threshold(model, X_test, y_test_binary, metric='f1')
...
def optimize_threshold(model, X_val, y_val, metric='f1'):
    ...
    print(f"이 임계값에서 - Precision: {results_df.loc[results_df['threshold'] == best_threshold, 'precision'].values[0]:.4f}, " 
...
def train_models(buy_list_db, craw_db, filtered_results, settings, threshold_method='recall'):
    """XGBoost 모델을 훈련합니다."""
    ...
    return best_model, best_accuracy, best_threshold
